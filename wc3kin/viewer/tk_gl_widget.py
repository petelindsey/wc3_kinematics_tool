...
class GLViewerFrame(tk.Frame):
    ...
    Adds a simple orbit camera:
      ...
      - Shift + MMB drag: pan center (planar in view plane)
      - Wheel: dolly
      - Ctrl+R: reset camera
    """
    ...
    def __init__(self, master: tk.Misc, **kwargs) -> None:
        ...
        class _Impl(OpenGLFrame):
            ...
            def initgl(self_inner) -> None:
                ...
                # toggles (bones shown by default)
            ...
            def _on_drag(self_inner, e: tk.Event) -> None:
                ...
                if mode == "pan":
                    # pan in camera screen plane (planar in view plane)
                    dist = float(self_inner._cam_dist)
                    ...
                    # Vector from center -> camera (matches redraw() cam_x/y/z computation)
                    fx = math.cos(pit) * math.sin(yaw)
                    fy = math.sin(pit)
                    fz = math.cos(pit) * math.cos(yaw)

                    # For screen-plane basis, we want view direction (camera -> center), i.e. NEGATE.
                    view_dir = GLViewerFrame._normalize((-fx, -fy, -fz))
                    world_up = (0.0, 1.0, 0.0)

                    # Right-handed screen basis: right = up × view_dir
                    right = GLViewerFrame._normalize(GLViewerFrame._cross(world_up, view_dir))
                    cam_up = GLViewerFrame._normalize(GLViewerFrame._cross(view_dir, right))

                    cx, cy, cz = self_inner._cam_center

                    # Dragging mouse right should move model right on screen => shift center left
                    cx -= right[0] * dx * pan_scale
                    cy -= right[1] * dx * pan_scale
                    cz -= right[2] * dx * pan_scale

                    # Dragging mouse down should move model down => shift center up (keep current feel)
        ...
        # note to chatgpt -- DO NOT CHANGE self._impl.animate = 0 NO MATTER WHAT
        self._impl.animate = 0 # DO NOT CHANGE THIS LINE!!!!!
        ...
        # ensure toggle exists even before initgl has run (defensive)
        self._impl._show_bones = True
    ...
    def set_pose(self, pose: Pose, rig: Rig, active_ids: Optional[set[int]] = None) -> None:
        ...
        #DO NOT REMOVE self._imple.request_redraw()
        self._impl.request_redraw()
        

    # ---- public render toggles ----
